...
public:
    ...
    void addEvaluationTarget(AILuaSolver::EvaluationTargetInfo * pEval)
    {
        m_pLuaSolver->m_pEvaluationTargets->addLast(pEval);
    };
    void addInterestForCurrentSpell(float fInterest)
    {
        m_pLuaSolver->m_fCurrentInterest += fInterest;
    };
    void addCurrentSpellInterestForDamages(Player * pPlayer, Unit * pUnit, u8 damages)
    {
        m_pLuaSolver->addCurrentSpellInterestForDamages(pPlayer, pUnit, damages);
    };
    Player * getCurrentPlayer()
    {
        return m_pLuaSolver->m_pCurrentPlayer;
    };
