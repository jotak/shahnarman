...
public:
  ...
  // Engines access
  DisplayEngine * getDisplay() { return m_pDisplayEngine; };
  InputEngine * getInput() { return m_pInputEngine; };

  // Managers access
  GameboardManager * getGameboard() { return m_pGameboardManager; };
  InterfaceManager * getInterface() { return m_pInterfaceManager; };
  FxManager * getFx() { return m_pFxManager; };
  AudioManager * getAudio() { return m_pAudioManager; };
  DebugManager * getDebug() { return m_pDebugManager; };
  PlayerManager * getPlayerManager() { return m_pPlayerManager; };
  u8 getClientId() { return m_uClientId; };

  // Other modules access
  Parameters * getClientParameters() { return m_pClientParameters; };
  DataFactory * getDataFactory() { return m_pDataFactory; };

  // Server related functions
  Server * initServer(wchar_t * sGameName, int nbClients, ClientData * clients, MapReader * pMapReader, int iTurnTimer, int iDeckSize);
  Server * loadServer(wchar_t * sGameName);
  Server * getServer() { return m_pServer; };
  void receiveMessage(NetworkData * pData);
  void sendMessage(NetworkData * pData);

  // Messages
  short getMessage() { short msg = m_iMessage; m_iMessage = MSG_NONE; return msg; };
  void requestExit() { m_iMessage = MSG_REQUEST_EXIT; };
  ...
  void endGame();
  int getTurnTimer() { return m_iTurnTimer; };
