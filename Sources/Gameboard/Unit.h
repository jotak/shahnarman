...
public:
    ...
    virtual u32 getType()
    {
        return MapObject::getType() | (m_Status == US_Dead ? GOTYPE_DEAD_UNIT : (m_Status == US_Removed ? GOTYPE_REMOVED_UNIT : GOTYPE_UNIT));
    };
    ...
    {
        snprintf(m_sIdentifiers, 16, "unit %d %ld", (int) m_uOwner, (long) m_uUnitId);
    };
    ...
    CoordsMap getPathTurnPosition()
    {
        return ((AStarStep*) m_pAStarPath->getCurrent(m_iPathTurnIterator))->pos;
    };
    ObjectList * getPath()
    {
        return m_pAStarPath;
    ...
    void setGroup(MetaObjectList * pGroup)
    {
        m_pGroup = pGroup;
    };
    MetaObjectList * getGroup()
    {
        return m_pGroup;
    };
    ObjectList * getSkillsRef()
    {
        return m_pSkillsRef;
    };
    void setPlayerColor(F_RGBA color)
    {
        m_PlayerColor = color;
    };
    ...
    bool canAttack()
    {
        return !m_bHasAttacked && m_Status == US_Normal;
    };
    bool canDefend()
    {
        return m_Status == US_Normal;
    };
    void onNewTurn();
    bool wasModified()
    {
