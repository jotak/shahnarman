...
MapObjectDlg::MapObjectDlg(LocalClient * pLocalClient, int iWidth, int iHeight) : guiDocument()
{
  ...
  // Objects list panel
  int yPxl = SPACING;
  m_pListPanel = guiContainer::createDefaultPanel(iWidth - 2 * SPACING, LIST_HEIGHT, L"ListPanel", pLocalClient->getDisplay());
  ...
  guiButton * pBtn = guiButton::createDefaultNormalButton(sText, L"OkButton", pLocalClient->getDisplay());
  ...
  m_pContentPanel = guiContainer::createDefaultPanel(iWidth - 2 * SPACING, pBtn->getYPos() - yPxl - 2 * SPACING, L"ContentPanel", pLocalClient->getDisplay());
  ...
  if (m_pBuildingPopup != NULL)
  {
    ...
    if (pCpnt != NULL)
    {
      if (wcscmp(pCpnt->getId(), L"OkButton") == 0)
      {
        ...
        guiGauge * pGauge = (guiGauge*) m_pContentPanel->getDocument()->getComponent(L"ProductionGauge");
        assert(pGauge != NULL);
        pGauge->setMax(cost);
...
bool MapObjectDlg::onButtonEvent(ButtonAction * pEvent, guiComponent * pCpnt)
{
  ...
  else if (wcscmp(pCpnt->getId(), L"ListButton") == 0)
  {
  ...
  else if (wcscmp(pCpnt->getId(), L"Buildable") == 0)
  {
  ...
  else if (wcscmp(pCpnt->getId(), L"UnitListButton") == 0)
  {
    ...
    if (pUnit != NULL)
    {
      ...
      guiGauge * pGauge = (guiGauge*) m_pContentPanel->getDocument()->getComponent(L"UnitProdGauge");
      assert(pGauge != NULL);
      pGauge->setMax(cost);
      pGauge->setValue(stock);
      swprintf_s(sText, LABEL_MAX_CHARS, L"%d / %d", stock, cost);
      ...
      wchar_t sUnitName[NAME_MAX_CHARS];
    ...
    else
    {
      ...
      pGauge->setMax(1);
      pGauge->setValue(0);
      pGauge->setTooltipText(L"");
      i18n->getText(L"PRODUCING_NOTHING", sText, LABEL_MAX_CHARS);
      guiLabel * pLbl = (guiLabel*) m_pContentPanel->getDocument()->getComponent(L"UnitProdGaugeLbl");
      assert(pLbl != NULL);
      pLbl->setText(sText);
      pLbl->centerOnComponent(pGauge);
      pLbl->setTooltipText(L"");
    }
  }
  return true;
}
...
guiObject * MapObjectDlg::onCursorMoveEvent(int xPxl, int yPxl)
{
  ...
  if (m_pContentPanel->isDocumentAt(xPxl, yPxl))
  {
    ...
    while (cpnt != NULL)
    {
      if (cpnt->isAt(xPxlRel, yPxlRel) && wcscmp(cpnt->getId(), L"Buildable") == 0)
      {
...
void MapObjectDlg::setTile(MapTile * pTile, bool bLoadObject)
{
  ...
  while (pObj != NULL)
  {
    guiToggleButton * pBtn = guiToggleButton::createDefaultTexturedToggleButton(pObj->getTexture(), m_pListPanel->getInnerHeight(), L"ListButton", getDisplay());
    pBtn->moveTo(xPxl, 0);
    ...
    if (pObj->getType() & GOTYPE_UNIT)
    {
      ...
      pImg->init(pPlayer->m_iBannerTex, L"", xPxl + 2*btnSize, 0, btnSize, btnSize, getDisplay());
...
void MapObjectDlg::loadUnit(Unit * pUnit)
{
  ...
  // Image
  guiImage * pImg = new guiImage();
  pImg->init(pUnit->getTexture(), L"", SPACING, SPACING, 100, 100, getDisplay());
  ...
  pLbl->init(sText, H1_FONT, H1_COLOR, L"", xPxl, yPxl, width, -1, getDisplay());
  ...
  pImg = new guiImage();
  pImg->init(pPlayer->m_iBannerTex, L"", pLbl->getXPos() - 36, yPxl+1, 32, 32, getDisplay());
  pImg->setDiffuseColor(pPlayer->m_Color);
  if (owner == 0)
    i18n->getText(L"NEUTRAL", sText, LABEL_MAX_CHARS);
  else
  {
  ...
  pLbl = new guiLabel();
  pLbl->init(sText, TEXT_FONT, TEXT_COLOR, L"", xPxl, yPxl, width, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pLbl);

  // Skills label
  yPxl = max(yPxl + pLbl->getHeight(), pImg->getYPos() + pImg->getHeight()) + 2 * SPACING;
  i18n->getText(L"SKILLS_AND_ACTIVE_EFFECTS", sText, LABEL_MAX_CHARS);
  pLbl = new guiLabel();
  ...
  guiContainer * pSkillsPanel = guiContainer::createDefaultPanel(panelWidth, panelHeight, L"SkillsPanel", getDisplay());
...
//    i18n->getText(L"%$1s_EFFECT_OF_%$2s", sText, LABEL_MAX_CHARS, pArgs);
    guiImage * pImg = new guiImage();
    pImg->init(getDisplay()->getTextureEngine()->loadTexture(pChild->sIcon), L"", xPxl, yPxl, ICON_SIZE, ICON_SIZE, getDisplay());
...
void MapObjectDlg::loadTown(Town * pTown)
{
  ...
  pImg->init(iTex, L"", pLbl->getXPos() - 36, yPxl+1, 32, 32, getDisplay());
  pImg->setDiffuseColor(pPlayer->m_Color);
  if (owner == 0)
    i18n->getText(L"NEUTRAL", sText, LABEL_MAX_CHARS);
  else
  ...
  pImg = new guiImage();
  pImg->init(pTown->getBigTexture(), L"", 0, 0, -1, -1, getDisplay());
  ...
  while (pBuilding != NULL)
  {
    ...
    if (pBuilding->isBuilt())
    {
      ...
      pImg = new guiImage();
      pImg->init(itex, L"Built", xPxl + pBuilding->getXPos(), yPxl + pBuilding->getYPos(), -1, -1, getDisplay());
    ...
    else if (pTown->isBuildingAllowed(pBuilding))
    {
      int itex = getDisplay()->getTextureEngine()->loadTexture(pBuilding->getIconPath());
      pImg = new guiImage();
      pImg->init(itex, L"Buildable", xPxl + pBuilding->getXPos(), yPxl + pBuilding->getYPos(), -1, -1, getDisplay());
  ...
  // Food icon
  pImg = new guiImage();
  pImg->init(getDisplay()->getTextureEngine()->loadTexture(L"food"), L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pImg);
  ...
  i18n->getText(L"GROWS_IN_%$1d_TURNS", sText, LABEL_MAX_CHARS, &p);
  pImg->setTooltipText(sText);
  ...
  pImg = new guiImage();
  pImg->init(getDisplay()->getTextureEngine()->loadTexture(L"happiness"), L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pImg);
  ...
  m_pContentPanel->getDocument()->addComponent(pLbl);
  if (happiness < -7)
    i18n->getText(L"CITIZENS_ARE_VERY_UNHAPPY", sText, LABEL_MAX_CHARS);
  else if (happiness < -4)
    i18n->getText(L"CITIZENS_ARE_UNHAPPY", sText, LABEL_MAX_CHARS);
  else if (happiness < 0)
    i18n->getText(L"CITIZENS_ARE_A_BIT_UNHAPPY", sText, LABEL_MAX_CHARS);
  else if (happiness == 0)
    i18n->getText(L"CITIZENS_ARE_NEUTRAL", sText, LABEL_MAX_CHARS);
  else if (happiness > 7)
    i18n->getText(L"CITIZENS_ARE_VERY_HAPPY", sText, LABEL_MAX_CHARS);
  else if (happiness > 4)
    i18n->getText(L"CITIZENS_ARE_HAPPY", sText, LABEL_MAX_CHARS);
  else
    i18n->getText(L"CITIZENS_ARE_A_BIT_HAPPY", sText, LABEL_MAX_CHARS);
  pImg->setTooltipText(sText);
  pLbl->setTooltipText(sText);
  ...
  xPxl += pLbl->getWidth() + 5 * SPACING;
  pImg = new guiImage();
  pImg->init(getDisplay()->getTextureEngine()->loadTexture(L"fear"), L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pImg);
  ...
  m_pContentPanel->getDocument()->addComponent(pLbl);
  if (fear <= 0)
    i18n->getText(L"FEAR_DESCRIPTION_NULL", sText, LABEL_MAX_CHARS);
  else if (fear < 3)
    i18n->getText(L"FEAR_DESCRIPTION_LOW", sText, LABEL_MAX_CHARS);
  else if (fear < 7)
    i18n->getText(L"FEAR_DESCRIPTION_AVERAGE", sText, LABEL_MAX_CHARS);
  else
    i18n->getText(L"FEAR_DESCRIPTION_HIGH", sText, LABEL_MAX_CHARS);
  pImg->setTooltipText(sText);
  pLbl->setTooltipText(sText);

  // Food gauge
  //int gaugeX = xPxl + pLbl->getWidth() + 25;
  //guiGauge * pGauge = guiGauge::createDefaultGauge(storage, rgb(1, 1, 0.3f), 200, 16, L"", getDisplay());
  //pGauge->setValue(stock);
  ...
  xPxl = SPACING;
  pImg = new guiImage();
  pImg->init(getDisplay()->getTextureEngine()->loadTexture(L"prod"), L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pImg);
  ...
  if (pBuilding != NULL)
  {
    ...
    pGauge = guiGauge::createDefaultGauge(cost, rgb(0.3f, 0.3f, 1), 200, 16, L"ProductionGauge", getDisplay());
    pGauge->setValue(stock);
    pGauge->moveTo(gaugeX, yPxl);
    m_pContentPanel->getDocument()->addComponent(pGauge);
    swprintf_s(sText, LABEL_MAX_CHARS, L"%d / %d", stock, cost);
  ...
  {
    pGauge = guiGauge::createDefaultGauge(1, rgb(0.3f, 0.3f, 1), 200, 16, L"ProductionGauge", getDisplay());
    pGauge->setValue(0);
    pGauge->moveTo(gaugeX, yPxl);
    m_pContentPanel->getDocument()->addComponent(pGauge);
    i18n->getText(L"BUILDING_NOTHING", sText, LABEL_MAX_CHARS);
    pLbl = new guiLabel();
    pLbl->init(sText, TEXT_FONT, TEXT_COLOR, L"ProductionGaugeLbl", 0, 0, -1, -1, getDisplay());
    pLbl->centerOnComponent(pGauge);
    m_pContentPanel->getDocument()->addComponent(pLbl);
  ...
  xPxl = SPACING;
  pImg = new guiImage();
  pImg->init(getDisplay()->getTextureEngine()->loadTexture(L"unitprod"), L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pImg);
  ...
  {
    pGauge = guiGauge::createDefaultGauge(1, rgb(1, 0.3f, 0.3f), 200, 16, L"UnitProdGauge", getDisplay());
    pGauge->moveTo(gaugeX, yPxl);
    pGauge->setValue(0);
    i18n->getText(L"PRODUCING_NOTHING", sText, LABEL_MAX_CHARS);
    pLbl = new guiLabel();
  ...
  yPxl = lowparty;
  i18n->getText(L"PRODUCE_UNIT", sText, LABEL_MAX_CHARS);
  pLbl = new guiLabel();
  ...
  yPxl += pLbl->getHeight() + SPACING;
  guiComboBox * pCombo = guiComboBox::createDefaultComboBox(L"UnitsList", m_pLocalClient->getInterface(), getDisplay());
  pCombo->moveTo(xPxl, yPxl);
  pCombo->setOwner(this);
  i18n->getText(L"NONE(UNITS_LIST)", sText, LABEL_MAX_CHARS);
  pCombo->addString(sText, L"UnitListButton");
  ...
  while (pName != NULL)
  {
    ...
    assert(pUnit != NULL);
    pUnit->findLocalizedElement(sText, LABEL_MAX_CHARS, i18n->getCurrentLanguageName(), L"name");
    guiButton * pComboBtn = pCombo->addString(sText, L"UnitListButton");
  ...
  while (pName != NULL)
  {
    ...
    guiButton * pComboBtn = pCombo->addString(sText, L"UnitListButton");
    pComboBtn->setAttachment(pName);
    wchar_t sInfo[LABEL_MAX_CHARS] = L"";
    pUnit->getInfos(sInfo, LABEL_MAX_CHARS, L"\n", false);
...
//  int x3rdCol = xPxl = 2 * m_pContentPanel->getInnerWidth() / 3 + SPACING;
  ...
  // Active effects
  i18n->getText(L"ACTIVE_EFFECTS", sText, LABEL_MAX_CHARS);
  pLbl = new guiLabel();
  pLbl->init(sText, H2_FONT, H2_COLOR, L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pContentPanel->getDocument()->addComponent(pLbl);
  ...
  yPxl += pLbl->getHeight() + SPACING;
  guiContainer * pPanel = guiContainer::createDefaultPanel(m_pContentPanel->getInnerWidth() / 3 - 2 * SPACING, m_pContentPanel->getInnerHeight() - yPxl - SPACING, L"ActiveEffects", getDisplay());
...
//    i18n->getText(L"%$1s_EFFECT_OF_%$2s", sText, LABEL_MAX_CHARS, pArgs);
  ...
  //yPxl += pImg->getHeight() + 4 * SPACING;
  //xPxl = x3rdCol;
  //i18n->getText(L"CHANCES_APPARITION_HEROE", sText, LABEL_MAX_CHARS);
  //pLbl = new guiLabel();
  //pLbl->init(sText, TEXT_FONT, TEXT_COLOR, L"", xPxl, yPxl, -1, -1, getDisplay());
  //m_pContentPanel->getDocument()->addComponent(pLbl);
  ...
  //pGauge = guiGauge::createDefaultGauge(1000, rgb(1.0f, 1.0f, 0), 200, 16, L"HeroeGauge", getDisplay());
...
void MapObjectDlg::raiseBuildingPopup(Building * pBuilding)
{
  ...
  setEnabled(false);
  m_pBuildingPopup = guiPopup::createOkCancelPopup(L"", m_pLocalClient->getDisplay());
  ...
  int itex = getDisplay()->getTextureEngine()->loadTexture(pBuilding->getIconPath());
  guiImage * pImg = new guiImage();
  pImg->init(itex, L"", xPxl, yPxl, -1, -1, getDisplay());
  ...
  xPxl += pImg->getWidth() + SPACING;
  guiLabel * pLbl = new guiLabel();
  pLbl->init(pBuilding->getLocalizedName(), H1_FONT, H1_COLOR, L"", xPxl, yPxl, width - xPxl, -1, getDisplay());
  ...
  // Prod. icon
  xPxl += pLbl->getWidth();
  itex = getDisplay()->getTextureEngine()->loadTexture(L"prod");
  pImg = new guiImage();
  pImg->init(itex, L"", xPxl, yPxl, -1, -1, getDisplay());
  m_pBuildingPopup->getDocument()->addComponent(pImg);
  ...
  pLbl = new guiLabel();
  pLbl->init(pBuilding->getLocalizedDescription(), H2_FONT, H2_COLOR, L"", xPxl, yPxl, width - xPxl, -1, getDisplay());
  m_pBuildingPopup->getDocument()->addComponent(pLbl);

  // Move buttons
  yPxl += pLbl->getHeight() + 3 * SPACING;
  guiComponent * pCpnt = m_pBuildingPopup->getDocument()->getComponent(L"OkButton");
  pCpnt->moveTo(3 * width / 4 - pCpnt->getWidth() / 2, yPxl);
  pCpnt->setAttachment(pBuilding);  // remember which building it is for later
  pCpnt = m_pBuildingPopup->getDocument()->getComponent(L"CancelButton");
...
void MapObjectDlg::setSelectedObject(MapObject * pObj)
{
  ...
  while (pCpnt != NULL)
  {
    if (wcscmp(pCpnt->getId(), L"ListButton") == 0)
    {
