...
public:
    ...
    MapTile * getTileAt(CoordsMap position)
    {
        if (!isInBounds(position)) return NULL;
        return m_pTiles[position.x][position.y];
    };
    ...
    MapObject * getFirstObjectAt(CoordsMap pos, u32 type = GOTYPE_MAPOBJECT)
    {
        return getFirstObjectAt(pos.x, pos.y, type);
    };
    MapObject * getNextObjectAt(CoordsMap pos, u32 type = GOTYPE_MAPOBJECT)
    {
        return getNextObjectAt(pos.x, pos.y, type);
    };
    int getObjectsCountAt(CoordsMap pos)
    {
        return getObjectsCountAt(pos.x, pos.y);
    };
