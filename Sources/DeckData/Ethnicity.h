...
public:
  ...
  {
  public:
    TownName(const char * pName) { wsafecpy(m_sName, NAME_MAX_CHARS, pName); m_bUsed = false; };
  ...
  {
  public:
    TownUnit(const char * sId, u8 uCost) { wsafecpy(m_sId, NAME_MAX_CHARS, sId); m_uCost = uCost; };
    char m_sId[NAME_MAX_CHARS];
    u8 m_uCost;
  };
  ...
  {
  public:
    TownHeroe(const char * sId) { wsafecpy(m_sId, NAME_MAX_CHARS, sId); m_bUsed = false; };
    char m_sId[NAME_MAX_CHARS];
    bool m_bUsed;
  };
