...
public:
  ...
  GameboardInputs * getInputs() { return m_pInputs; };
  int getBattleTexture() { return m_pBattleIcon->getTexture(); };
  MapCursor * getMapCursor(int iCursor) { return &(m_AllCursors[iCursor]); };
  ...
  bool isCurrentTargetValid() { return m_AllCursors[MAPCURSOR_MOVETARGET].isEnabled(); };
  void updateTownsData(NetworkData * pData);
  void updateTilesInfluence(NetworkData * pData);
  void unsetTargetMode();
  ObjectList * getMagicCircles() { return m_pMagicCircles; };
